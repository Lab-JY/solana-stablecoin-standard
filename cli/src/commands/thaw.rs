...
/// Anchor instruction discriminator for `thaw_account`.
/// sha256("global:thaw_account")[..8]
const THAW_DISCRIMINATOR: [u8; 8] = [115, 152, 79, 213, 213, 169, 184, 35];
...
pub async fn execute(cfg: &CliConfig, mint_addr: &str, address: &str) -> Result<()> {
    ...
    let data = THAW_DISCRIMINATOR.to_vec();
    ...
    let accounts = vec![
        ...
        AccountMeta::new(target_ata, false),             // token account to thaw (mut)
    ...
    let signature = rpc
        ...
        .context("Sending thaw transaction")?;

    output::header("Account Thawed");
    ...
    output::success("Account thawed successfully");
